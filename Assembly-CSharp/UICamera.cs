...
public class UICamera : MonoBehaviour
{
		...
		set
		{
			...
			if (mSel != null)
			{
				...
				if (uICamera != null)
				{
					...
					{
						Highlight(mSel, highlighted: false);
					}
					current = null;
				}
			}
			...
			if (uICamera2 != null)
			{
				...
				{
					Highlight(mSel, highlighted: true);
				}
	...
	private void Awake()
	{
		...
		ref Vector2 pos = ref mMouse[0].pos;
		Vector3 mousePosition = Input.mousePosition;
		pos.x = mousePosition.x;
		ref Vector2 pos2 = ref mMouse[0].pos;
		Vector3 mousePosition2 = Input.mousePosition;
		pos2.y = mousePosition2.y;
	...
	public void ProcessMouse()
	{
		...
		if (useMouse && !flag3 && mHover != null && mHover != mMouse[0].current)
		{
			...
			Highlight(mHover, highlighted: false);
			mHover = null;
		}
		if (useMouse)
		{
		...
		if (useMouse && !flag3 && mHover != mMouse[0].current)
		{
			...
			Highlight(mHover, highlighted: true);
		}
	}
	...
	public void ProcessTouch(bool pressed, bool unpressed)
	{
		...
		{
			if (currentTouch.clickNotification != 0 && !stickyPress && !unpressed && currentTouch.pressStarted && currentTouch.pressed != hoveredObject)
			{
				isDragging = true;
				Notify(currentTouch.pressed, "OnPress", false);
