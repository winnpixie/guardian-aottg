...
public class FengGameManagerMKII : Photon.MonoBehaviour, Anarchy.Custom.Interfaces.IGameManager
{
    ...
    {
        // TODO: Mod
        if (Guardian.Mod.Instance.Muted.Contains(info.sender.Id))
        {
            return;
        }
    ...
    private void OnLevelWasLoaded(int level)
    {
        ...
        Camera.main.GetComponent<IN_GAME_MAIN_CAMERA>().SetLighting(IN_GAME_MAIN_CAMERA.Lighting);
        ...
        if (PhotonNetwork.isMasterClient)
        {
            switch (Level.Mode)
            {
                ...
                case GameMode.PVP_CAPTURE:
                    ...
                    {
                        GameObject[] titanRespawns = GameObject.FindGameObjectsWithTag("titanRespawn");
                        for (int i = 0; i < titanRespawns.Length; i++)
                        {
                            SpawnTitanRaw(titanRespawns[i].transform.position, titanRespawns[i].transform.rotation).GetComponent<TITAN>().setAbnormalType2(TitanClass.Crawler, forceCrawler: true);
                        }
                    }
                    break;
            }
    ...
    public void OnPhotonPlayerPropertiesChanged(object[] playerAndUpdatedProps)
    {
        ...
        if (properties.ContainsKey("sender") && properties["sender"] is PhotonPlayer)
        {
            sender = (PhotonPlayer)properties["sender"];
        }

        if (player.isLocal && sender != null && !sender.isLocal)
        {
    ...
    private void Core2()
    {
        ...
        else if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Singleplayer) // Singleplayer messages
        {
            ...
            if (isWinning)
            {
                ...
                {
                    case GameMode.RACING:
                        ShowHUDInfoCenter(Guardian.Utilities.GameHelper.FormatTime((timeTotalServer * 10f) * 0.1f - 5f, true) + "!\nPress " + inputManager.inputString[InputCode.Restart] + " to restart.");
                        break;
                    case GameMode.SURVIVE_MODE:
                        ShowHUDInfoCenter("Survived All Waves!\nPress " + inputManager.inputString[InputCode.Restart] + " to restart.");
                        break;
        ...
        if (Level.Mode == GameMode.RACING)
        {
            if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Singleplayer)
            {
                if (!isWinning)
                {
                    ShowHUDInfoTopCenter("Time : " + Guardian.Utilities.GameHelper.FormatTime(timeTotalServer * 10 * 0.1f - 5f, true));
                }
                if (timeTotalServer < 5f)
                {
                    ShowHUDInfoCenter("RACE START IN " + Guardian.Utilities.GameHelper.FormatTime(5f - timeTotalServer, true));
                }
                else if (!startRacing)
                {
                    ShowHUDInfoCenter(string.Empty);
            ...
            {
                ShowHUDInfoTopCenter("Time : " + (roundTime >= 20f ? Guardian.Utilities.GameHelper.FormatTime(roundTime * 10f * 0.1f - 20f, true) : "WAITING"));
                if (roundTime < 20f)
                {
                    ShowHUDInfoCenter("RACE START IN " + Guardian.Utilities.GameHelper.FormatTime(20f - roundTime, true) + (localRacingResult.Length > 0 ? ("\nLast Round\n" + localRacingResult) : string.Empty));
                }
                else if (!startRacing)
                {
                    ShowHUDInfoCenter(string.Empty);
        ...
        if (timeElapse > 1f)
        {
            ...
            {
                ShowHUDInfoTopRightMAPNAME(Guardian.Utilities.GameHelper.FormatTime(roundTime) + "\n" + Level.Name + " : " + difficultyTxt);
            }
            else
            {
    ...
    private IEnumerator CoRespawn(float seconds)
    {
        while (true)
        {
            ...
            foreach (PhotonPlayer player in PhotonNetwork.playerList)
            {
                if (player.customProperties[PhotonPlayerProperty.RCTeam] == null && GExtensions.AsBool(player.customProperties[PhotonPlayerProperty.Dead]) && GExtensions.AsInt(player.customProperties[PhotonPlayerProperty.IsTitan]) != 2)
                {
    ...
    public void OnGUI()
    {
        ...
        else
        {
            ...
            else if (inputManager != null && inputManager.menuOn)
            {
                ...
                else if (GUI.Button(new Rect(halfMenuWidth + 496f, halfMenuHeight + 7f, 88f, 25f), "Custom Logic"))
                {
                    Settings[64] = 9;
                }
                else if (GUI.Button(new Rect(halfMenuWidth + 589f, halfMenuHeight + 7f, 95f, 25f), "Game Settings"))
                {
                    Settings[64] = 10;
                }
                ...
                switch ((int)Settings[64])
                {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        GUI.TextArea(new Rect(halfMenuWidth + 80f, halfMenuHeight + 52f, 270f, 30f), "Settings saved to playerprefs!", 100, "Label");
                        break;
                    case 5:
                        GUI.TextArea(new Rect(halfMenuWidth + 80f, halfMenuHeight + 52f, 270f, 30f), "Settings reloaded from playerprefs!", 100, "Label");
                        break;
                    case 6:
                        // ?
                        break;
                    case 7:
                        {
                            ...
                            if ((int)Settings[188] == 0)
                            {
                                ...
                                scroll = GUI.BeginScrollView(new Rect(halfMenuWidth, halfMenuHeight + 115f, 712f, 340f), scroll, new Rect(halfMenuWidth, halfMenuHeight + 115f, 700f, 475f), alwaysShowHorizontal: true, alwaysShowVertical: true);
                ...
                if (GUI.Button(new Rect(halfMenuWidth + 408f, halfMenuHeight + 465f, 42f, 25f), "Save"))
                {
                ...
                else if (GUI.Button(new Rect(halfMenuWidth + 455f, halfMenuHeight + 465f, 40f, 25f), "Load"))
                {
                    LoadConfig();
                    Settings[64] = 5;
                }
                else if (GUI.Button(new Rect(halfMenuWidth + 500f, halfMenuHeight + 465f, 60f, 25f), "Default"))
                {
                    GameObject.Find("InputManagerController").GetComponent<FengCustomInputs>().setToDefault();
                }
                else if (GUI.Button(new Rect(halfMenuWidth + 565f, halfMenuHeight + 465f, 75f, 25f), "Continue"))
                {
                    if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Singleplayer)
                    {
                        Time.timeScale = 1f;
                ...
                else if (GUI.Button(new Rect(halfMenuWidth + 645f, halfMenuHeight + 465f, 40f, 25f), "Quit"))
                {
                    if (IN_GAME_MAIN_CAMERA.Gametype == GameType.Singleplayer)
                    {
                        Time.timeScale = 1f;
            ...
            case 1: // Cyan
                {
                    ...
                    hashtable.Add(PhotonPlayerProperty.Name, "[00FFFF]" + LoginFengKAI.Player.Name.Uncolored());
                    PhotonNetwork.player.SetCustomProperties(hashtable);
                    break;
                }
            case 2: // Magenta
                {
                    ...
                    hashtable.Add(PhotonPlayerProperty.Name, "[FF00FF]" + LoginFengKAI.Player.Name.Uncolored());
                    PhotonNetwork.player.SetCustomProperties(hashtable);
                    break;
                }
            case 3: // Auto
