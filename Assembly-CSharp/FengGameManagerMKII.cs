...
public class FengGameManagerMKII : Photon.MonoBehaviour
{
    ...
    {
        get { return PhotonNetwork.isMasterClient ? time < 0 ? (time * 2) : 0 : _timeTotalServer; }
    ...
    public void oneTitanDown(string name1, bool onPlayerLeave)
    {
        ...
        switch (IN_GAME_MAIN_CAMERA.Gamemode)
        {
            ...
            case GAMEMODE.SURVIVE_MODE:
                ...
                {
                    sendChatContentInfo("<color=#A8FF24>Wave </color>" + wave + " / " + RCSettings.GetMaxWave());
                }
                if (wave > highestWave)
                {
                    highestWave = wave;
                ...
                if (!LevelInfo.getInfo(level).punk || wave % 5 != 0)
                {
        ...
        if (PhotonNetwork.isMasterClient)
        {
            switch (info.type)
            {
                ...
                case GAMEMODE.SURVIVE_MODE:
                    ...
                    break;
                case GAMEMODE.PVP_CAPTURE:
                    if (LevelInfo.getInfo(FengGameManagerMKII.level).mapName == "OutSide")
                    {
    ...
    public void OnPhotonPlayerConnected(PhotonPlayer player)
    {
        if (PhotonNetwork.isMasterClient)
        {
            ...
            else
            {
                ...
                {
                    StartCoroutine(CustomLevelE(new List<PhotonPlayer>
                    {
                        player
                    }));
                }
    ...
    private void core2()
    {
        ...
        if (timeElapse > 1f)
        {
            ...
            {
                case GAMEMODE.ENDLESS_TITAN:
                    text = "Time : " + (time - (int)timeTotalServer).ToString();
                    break;
                case GAMEMODE.KILL_TITAN:
                case GAMEMODE.None:
                    text = "Titan Left: " + GameObject.FindGameObjectsWithTag("titan").Length + "  Time : ";
                    text = ((IN_GAME_MAIN_CAMERA.Gametype != 0) ? (text + (time - (int)timeTotalServer)) : (text + ((int)timeTotalServer)));
                    break;
                case GAMEMODE.SURVIVE_MODE:
                    text = "Titan Left: ";
                    text += GameObject.FindGameObjectsWithTag("titan").Length.ToString() + " Wave : " + wave;
                    break;
                case GAMEMODE.BOSS_FIGHT_CT:
                    text = "Time : ";
                    text += (time - (int)timeTotalServer).ToString() + "\nDefeat the Colossal Titan\nand prevent abnormal titans from reaching the north gate!";
                    break;
                case GAMEMODE.PVP_CAPTURE:
                    string str = "| ";
            ...
            if (IN_GAME_MAIN_CAMERA.Gametype == GAMETYPE.SINGLE)
            {
                ...
                {
                    text = "Time : ";
                    text += ((int)timeTotalServer).ToString();
                }
            }
            else
            {
                switch (IN_GAME_MAIN_CAMERA.Gamemode)
                {
                    ...
                    case GAMEMODE.PVP_CAPTURE:
                        ...
                        break;
                    case GAMEMODE.SURVIVE_MODE:
                        text = "Time : " + (time - (int)timeTotalServer);
                        break;
                    case GAMEMODE.PVP_AHSS:
            ...
            string text3 = (IN_GAME_MAIN_CAMERA.Difficulty < 0) ? "Training" : ((IN_GAME_MAIN_CAMERA.Difficulty == 0) ? "Normal" : ((IN_GAME_MAIN_CAMERA.Difficulty != 1) ? "Abnormal" : "Hard"));
            ...
            if (IN_GAME_MAIN_CAMERA.Gametype == GAMETYPE.MULTIPLAYER)
            {
                ...
                if (text4.Length > 20)
                {
                    text4 = text4.Remove(19) + "...";
                }
                ShowHUDInfoTopRightMAPNAME("\n" + text4 + " [ffcc00](" + Convert.ToString(PhotonNetwork.room.playerCount) + "/" + Convert.ToString(PhotonNetwork.room.maxPlayers) + ")");
    ...
    private void refreshRacingResult2()
    {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            localRacingResult = localRacingResult + "   " + ((float)(int)((racingResult[i] as RacingResult).time * 100f) * 0.01f).ToString() + "s";
            localRacingResult += "\n";
        }
    ...
    private string GetPlayerTextForList(PhotonPlayer player)
    {
        ...
        content += $"{ModDetector.GetMod(player)}[-] ";
            ...
            if (OldScript != CurrentScript)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < Mathf.FloorToInt((array5.Length - 1) / 100) + 1; i++)
                    {
                        ...
                        for (int j = 100 * i; j < 100 * i + array5.Length % 100; j++)
                        {
                            if (array5[j].StartsWith("spawnpoint"))
                            {
                                ...
                                else if (array7[1] == "playerC")
                                {
                                    playerSpawnsC.Add(new Vector3(Convert.ToSingle(array7[2]), Convert.ToSingle(array7[3]), Convert.ToSingle(array7[4])));
                                }
                                else if (array7[1] == "playerM")
                                {
                                    playerSpawnsM.Add(new Vector3(Convert.ToSingle(array7[2]), Convert.ToSingle(array7[3]), Convert.ToSingle(array7[4])));
                                }
                            }
                    ...
                    foreach (Vector3 titanSpawn in titanSpawns)
                    {
                        ...
                        array9[1] = x.ToString();
                        array8[2] = ",";
                        string[] array10 = array8;
                        x = titanSpawn.y;
                        array10[3] = x.ToString();
                        array8[4] = ",";
                        string[] array11 = array8;
                        x = titanSpawn.z;
                        array11[5] = x.ToString();
                    ...
                    foreach (Vector3 item in playerSpawnsC)
                    {
                        ...
                        float x = item.x;
                        array12[1] = x.ToString();
                        array8[2] = ",";
                        string[] array13 = array8;
                        x = item.y;
                        array13[3] = x.ToString();
                        array8[4] = ",";
                        string[] array14 = array8;
                        x = item.z;
                        array14[5] = x.ToString();
                        list.Add(string.Concat(array8));
                    }
                    foreach (Vector3 item2 in playerSpawnsM)
                    {
                        ...
                        string[] array15 = array8;
                        float x = item2.x;
                        array15[1] = x.ToString();
                        array8[2] = ",";
                        string[] array16 = array8;
                        x = item2.y;
                        array16[3] = x.ToString();
                        array8[4] = ",";
                        string[] array17 = array8;
                        x = item2.z;
                        array17[5] = x.ToString();
                        list.Add(string.Concat(array8));
                    }
    ...
    private IEnumerator ClearLevelE(string[] skybox)
    {
        ...
        if (skybox[0] != string.Empty || skybox[1] != string.Empty || skybox[2] != string.Empty || skybox[3] != string.Empty || skybox[4] != string.Empty || skybox[5] != string.Empty)
        {
            ...
            if (!LinkHash[1].ContainsKey(key))
            {
                ...
                string sky0 = skybox[0];
                string sky = skybox[1];
                string sky2 = skybox[2];
                string sky3 = skybox[3];
                string sky4 = skybox[4];
                string sky5 = skybox[5];
                if (sky0.EndsWith(".jpg") || sky0.EndsWith(".png") || sky0.EndsWith(".jpeg"))
                {
                    WWW www = GameHelper.CreateWWW(sky0);
                    if (www != null)
                    {
                        yield return www;
                ...
                if (sky.EndsWith(".jpg") || sky.EndsWith(".png") || sky.EndsWith(".jpeg"))
                {
                    WWW www = GameHelper.CreateWWW(sky);
                    if (www != null)
                    {
                        yield return www;
                ...
                if (sky2.EndsWith(".jpg") || sky2.EndsWith(".png") || sky2.EndsWith(".jpeg"))
                {
                    WWW www = GameHelper.CreateWWW(sky2);
                    if (www != null)
                    {
                        yield return www;
                ...
                if (sky3.EndsWith(".jpg") || sky3.EndsWith(".png") || sky3.EndsWith(".jpeg"))
                {
                    WWW www = GameHelper.CreateWWW(sky3);
                    if (www != null)
                    {
                        yield return www;
                ...
                if (sky4.EndsWith(".jpg") || sky4.EndsWith(".png") || sky4.EndsWith(".jpeg"))
                {
                    WWW www = GameHelper.CreateWWW(sky4);
                    if (www != null)
                    {
                        yield return www;
                ...
                if (sky5.EndsWith(".jpg") || sky5.EndsWith(".png") || sky5.EndsWith(".jpeg"))
                {
                    WWW www = GameHelper.CreateWWW(sky5);
                    if (www != null)
                    {
                        yield return www;
        ...
        else
        {
            ...
            else
            {
                ...
                if (!LogicLoaded || !CustomLevelLoaded)
                {
                    ...
                    GUI.Label(new Rect(num7 - 60f, num8 - 30f, 200f, 22f), "Loading Level (" + length.ToString() + "/" + length2.ToString() + ")");
    ...
    private void SetGameSettings(ExitGames.Client.Photon.Hashtable settings)
    {
        ...
        if (settings.ContainsKey("point"))
        {
            if (RCSettings.PointMode != (int)settings["point"])
            {
                ...
                chatRoom.AddLine("<color=#ffcc00>Point limit enabled (" + Convert.ToString(RCSettings.PointMode) + ").</color>");
            }
        }
        else if (RCSettings.PointMode != 0)
        {
        ...
        if (settings.ContainsKey("explode"))
        {
            if (RCSettings.ExplodeMode != (int)settings["explode"])
            {
                ...
                chatRoom.AddLine("<color=#ffcc00>Titan Explode Mode enabled (Radius " + Convert.ToString(RCSettings.ExplodeMode) + ").</color>");
            }
        }
        ...
        if (settings.ContainsKey("maxwave"))
        {
            if (RCSettings.MaxWave != (int)settings["maxwave"])
            {
                ...
                chatRoom.AddLine("<color=#ffcc00>Max wave is " + RCSettings.MaxWave.ToString() + ".</color>");
            }
        }
        ...
        if (settings.ContainsKey("endless"))
        {
            if (RCSettings.EndlessMode != (int)settings["endless"])
            {
                ...
                chatRoom.AddLine("<color=#ffcc00>Endless Respawn enabled (" + RCSettings.EndlessMode.ToString() + " seconds).</color>");
            }
        }
